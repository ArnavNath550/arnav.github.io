//@ts-nocheck
import * as React from "react";
import styled from "styled-components";
import { motion, AnimatePresence } from "framer-motion";
import PageBack from "../PageBack";

const FamilyTrayAttemptComponent: React.FC = () => {
  const [isTrayOpen, setIsTrayOpen] = React.useState(false);
  return (
    <Container>
      <PageBack pageName="Craft" pagePath="/craft" />
      <div style={{ width: 150 }}>
        <AnimatedButton onClick={() => setIsTrayOpen(!isTrayOpen)}>
          Open Tray
        </AnimatedButton>
      </div>
      <Tray isTrayOpen={isTrayOpen} setIsTrayOpen={setIsTrayOpen} />
    </Container>
  );
};

export default FamilyTrayAttemptComponent;

const trayVariants = {
  initial: {
    opacity: 0.5,
    y: 10,
  },
  animate: {
    opacity: 1,
    y: 0,
    scale: 1,
    transition: {
      duration: 0.1,
      ease: [0.16, 1, 0.3, 1],
    },
  },
  exit: {
    opacity: 0.5,
    y: -5,
    scale: 0.98,
    transition: {
      duration: 0.15,
      ease: [0.4, 0, 0.2, 1],
    },
  },
};

const contentVariants = {
  initial: {
    opacity: 0,
    scale: 0.9,
    y: 5, // Much less y movement
  },
  animate: {
    opacity: 1,
    y: 0,
    scale: 1,
    transition: {
      duration: 0.1,
      ease: [0.25, 0.1, 0.25, 1], // Smoother ease, no spring
      opacity: { duration: 0.2 },
      scale: { duration: 0.2 },
      y: { duration: 0.2 },
    },
  },
  exit: {
    opacity: 0,
    scale: 0.98,
    y: 0, // No y movement on exit
    transition: {
      duration: 0.2,
      ease: [0.4, 0, 0.6, 1],
    },
  },
};

type TrayProps = {
  isTrayOpen: boolean;
  setIsTrayOpen: () => void;
};

export const Tray: React.FC = (props: TrayProps) => {
  const [content, setContent] = React.useState(-1);
  const [trayHeight, setTrayHeight] = React.useState(0);
  const contentRef = React.useRef<HTMLDivElement>(null);

  const [actions] = React.useState([
    {
      icon: (
        <svg
          width="20"
          height="21"
          viewBox="0 0 20 21"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M6.00024 9V6C6.00024 3.79086 7.79111 2 10.0002 2V2C12.2094 2 14.0002 3.79086 14.0002 6V9"
            stroke="#8F8F8F"
            strokeWidth="2.33319"
          ></path>
          <path
            d="M6.68423 9H13.3163V7H6.68423V9ZM16.0002 11.684V16.316H18.0002V11.684H16.0002ZM13.3163 19H6.68423V21H13.3163V19ZM4.00024 16.316V11.684H2.00024V16.316H4.00024ZM6.68423 19C5.20191 19 4.00024 17.7983 4.00024 16.316H2.00024C2.00024 18.9029 4.09734 21 6.68423 21V19ZM16.0002 16.316C16.0002 17.7983 14.7986 19 13.3163 19V21C15.9032 21 18.0002 18.9029 18.0002 16.316H16.0002ZM13.3163 9C14.7986 9 16.0002 10.2017 16.0002 11.684H18.0002C18.0002 9.09709 15.9032 7 13.3163 7V9ZM6.68423 7C4.09734 7 2.00024 9.09709 2.00024 11.684H4.00024C4.00024 10.2017 5.20191 9 6.68423 9V7Z"
            fill="#8F8F8F"
          ></path>
        </svg>
      ),
      actionIcon: (
        <svg
          width="48"
          height="48"
          viewBox="0 0 48 48"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M16.4517 37C15.0283 36.9773 13.5916 36.9488 12.2178 36.9183C9.16956 36.8506 7.64544 36.8167 6.41564 36.1889C5.31269 35.6258 4.35358 34.6681 3.78881 33.566C3.15908 32.3372 3.12349 30.8373 3.0523 27.8376C3.02068 26.5048 3 25.1888 3 24.0259C3 22.8629 3.02068 21.5469 3.0523 20.2142C3.12349 17.2144 3.15908 15.7145 3.78881 14.4857C4.35358 13.3836 5.31269 12.4259 6.41564 11.8628C7.64544 11.235 9.16959 11.2011 12.2179 11.1334C15.4835 11.0609 19.1045 11 22.0566 11C25.6932 11 30.345 11.0924 34.0971 11.1852C35.0534 11.2088 35.5316 11.2206 35.9699 11.2967C38.4232 11.7225 40.436 13.7225 40.8775 16.173C40.9564 16.6107 40.9709 17.078 41 18.0126V18.0126V18.5916"
            stroke="#999999"
            stroke-width="2.75"
            stroke-linecap="round"
          ></path>
          <path
            d="M10 24.0001H19"
            stroke="#999999"
            stroke-width="2.75"
            stroke-linecap="round"
          ></path>
          <path
            d="M10 30H16"
            stroke="#999999"
            stroke-width="2.75"
            stroke-linecap="round"
          ></path>
          <path
            d="M36.8607 32.1238C36.8607 34.2665 35.1086 36.0036 32.9472 36.0036C30.7859 36.0036 29.0337 34.2665 29.0337 32.1238C29.0337 29.981 30.7859 28.2439 32.9472 28.2439C35.1086 28.2439 36.8607 29.981 36.8607 32.1238Z"
            stroke="#999999"
            stroke-width="2.75"
            stroke-linecap="round"
            stroke-linejoin="round"
          ></path>
          <path
            d="M21.009 33.4638C20.6293 32.6114 20.6293 31.636 21.009 30.7836C23.0327 26.241 27.6168 23.0708 32.9478 23.0708C38.2788 23.0708 42.8629 26.2411 44.8866 30.7837C45.2664 31.6361 45.2664 32.6115 44.8866 33.4639C42.8629 38.0065 38.2788 41.1767 32.9478 41.1767C27.6168 41.1767 23.0327 38.0064 21.009 33.4638Z"
            stroke="#999999"
            stroke-width="2.75"
            stroke-linecap="round"
            stroke-linejoin="round"
          ></path>
        </svg>
      ),
      actionText: "Private Key",
      actionDisplayText: "Private Key",
      type: "netural",
      content: 0,
      actionDescription:
        "Your Private Key is the key used to back up your wallet. Keep it secret and secure at all times.",
      height: 440,
      listItems: [
        {
          icon: (
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M3.08654 6.41197C3.13639 5.95634 3.5287 5.62685 3.98583 5.59345C6.79146 5.38847 9.00727 4.30249 11.0607 2.59943C11.4593 2.26879 12.0405 2.26879 12.4391 2.59944C14.4925 4.30249 16.7083 5.38846 19.5139 5.59345C19.9711 5.62685 20.3634 5.95638 20.4132 6.41203C20.4704 6.93468 20.4998 7.46586 20.4998 8.00402C20.4998 13.9495 17.4683 19.0446 12.4223 21.1815C11.9928 21.3634 11.507 21.3634 11.0775 21.1815C6.03148 19.0446 3 13.9495 3 8.00402C3 7.46584 3.02936 6.93464 3.08654 6.41197Z"
                stroke="#A5A5A5"
                strokeWidth="2"
              ></path>
              <path
                d="M8.49097 11.7295L10.8281 14.0359L15.5023 9.4232"
                stroke="#A5A5A5"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></path>
            </svg>
          ),
          listText: "Keep your Private Key safe",
          height: 440,
        },
        {
          icon: (
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M1.75 12L2.75 12L1.75 12ZM12 20.25L12 21.25L12 20.25ZM5.15863 20.1131L5.19061 19.1136L5.15863 20.1131ZM22.1376 7.15748L23.1369 7.11999L22.1376 7.15748ZM2.86171 16.805C2.80378 15.2613 2.75 13.4384 2.75 12L0.75 12C0.75 13.4772 0.804975 15.3306 0.863116 16.88L2.86171 16.805ZM2.75 12C2.75 10.5616 2.80378 8.73874 2.86171 7.19498L0.863117 7.11999C0.804975 8.66944 0.75 10.5228 0.75 12L2.75 12ZM5.19061 4.88636C7.25799 4.82021 9.93991 4.75 12 4.75L12 2.75C9.90598 2.75 7.1972 2.82114 5.12666 2.88738L5.19061 4.88636ZM12 4.75C14.0601 4.75 16.742 4.82021 18.8094 4.88636L18.8733 2.88738C16.8028 2.82114 14.094 2.75 12 2.75L12 4.75ZM21.1383 7.19498C21.1962 8.73874 21.25 10.5616 21.25 12L23.25 12C23.25 10.5228 23.195 8.66944 23.1369 7.11999L21.1383 7.19498ZM21.25 12C21.25 13.4384 21.1962 15.2613 21.1383 16.805L23.1369 16.88C23.195 15.3306 23.25 13.4772 23.25 12L21.25 12ZM18.8094 19.1136C16.742 19.1798 14.0601 19.25 12 19.25L12 21.25C14.094 21.25 16.8028 21.1789 18.8733 21.1126L18.8094 19.1136ZM12 19.25C9.93991 19.25 7.25799 19.1798 5.19061 19.1136L5.12666 21.1126C7.1972 21.1789 9.90598 21.25 12 21.25L12 19.25ZM2.86171 7.19498C2.90906 5.93324 3.92285 4.92692 5.19061 4.88636L5.12666 2.88738C2.81454 2.96136 0.950076 4.80253 0.863117 7.11999L2.86171 7.19498ZM0.863116 16.88C0.950076 19.1975 2.81454 21.0386 5.12666 21.1126L5.19061 19.1136C3.92285 19.0731 2.90906 18.0668 2.86171 16.805L0.863116 16.88ZM21.1383 16.805C21.0909 18.0668 20.0772 19.0731 18.8094 19.1136L18.8733 21.1126C21.1855 21.0386 23.0499 19.1975 23.1369 16.88L21.1383 16.805ZM18.8094 4.88636C20.0772 4.92692 21.0909 5.93324 21.1383 7.19498L23.1369 7.11999C23.0499 4.80253 21.1855 2.96136 18.8733 2.88738L18.8094 4.88636Z"
                fill="#A5A5A5"
              ></path>
              <rect
                x="5.49951"
                y="7.12207"
                width="5.85"
                height="1.95"
                rx="0.975"
                fill="#A5A5A5"
              ></rect>
              <rect
                x="5.49951"
                y="11.0234"
                width="5.85"
                height="1.95"
                rx="0.975"
                fill="#A5A5A5"
              ></rect>
              <rect
                x="5.49951"
                y="14.9248"
                width="5.85"
                height="1.95"
                rx="0.975"
                fill="#A5A5A5"
              ></rect>
              <rect
                x="12.6504"
                y="7.12207"
                width="5.85"
                height="1.95"
                rx="0.975"
                fill="#A5A5A5"
              ></rect>
              <rect
                x="12.6504"
                y="11.0234"
                width="5.85"
                height="1.95"
                rx="0.975"
                fill="#A5A5A5"
              ></rect>
              <rect
                x="12.6504"
                y="14.9248"
                width="5.85"
                height="1.95"
                rx="0.975"
                fill="#A5A5A5"
              ></rect>
            </svg>
          ),
          listText: "Don't share it with anyone else",
        },
        {
          icon: (
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M12 2.99976C7.02944 2.99976 3 7.02919 3 11.9998C3 16.9703 7.02944 20.9998 12 20.9998C16.9706 20.9998 21 16.9703 21 11.9998C21 7.02919 16.9706 2.99976 12 2.99976Z"
                stroke="#A5A5A5"
                strokeWidth="2.2"
              ></path>
              <path
                d="M5.63599 5.63602L18.3639 18.3639"
                stroke="#A5A5A5"
                strokeWidth="2.2"
              ></path>
            </svg>
          ),
          listText: "If you lose it, we can't recover it",
        },
      ],
    },
    {
      icon: (
        <svg
          width="24"
          height="20"
          viewBox="0 0 24 20"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M1.75049 10L2.75049 10L1.75049 10ZM12.0005 18.25L12.0005 19.25L12.0005 18.25ZM5.15912 18.1131L5.1911 17.1136L5.15912 18.1131ZM22.1381 5.15748L23.1374 5.11999L22.1381 5.15748ZM2.8622 14.805C2.80427 13.2613 2.75049 11.4384 2.75049 10L0.750488 10C0.750488 11.4772 0.805463 13.3306 0.863604 14.88L2.8622 14.805ZM2.75049 10C2.75049 8.56162 2.80427 6.73874 2.8622 5.19498L0.863605 5.11999C0.805463 6.66944 0.750488 8.5228 0.750488 10L2.75049 10ZM5.1911 2.88636C7.25848 2.82021 9.9404 2.75 12.0005 2.75L12.0005 0.75C9.90647 0.75 7.19769 0.821136 5.12715 0.887381L5.1911 2.88636ZM12.0005 2.75C14.0606 2.75 16.7425 2.82021 18.8099 2.88636L18.8738 0.887382C16.8033 0.821136 14.0945 0.75 12.0005 0.75L12.0005 2.75ZM21.1388 5.19498C21.1967 6.73874 21.2505 8.56162 21.2505 10L23.2505 10C23.2505 8.5228 23.1955 6.66944 23.1374 5.11999L21.1388 5.19498ZM21.2505 10C21.2505 11.4384 21.1967 13.2613 21.1388 14.805L23.1374 14.88C23.1955 13.3306 23.2505 11.4772 23.2505 10L21.2505 10ZM18.8099 17.1136C16.7425 17.1798 14.0606 17.25 12.0005 17.25L12.0005 19.25C14.0945 19.25 16.8033 19.1789 18.8738 19.1126L18.8099 17.1136ZM12.0005 17.25C9.9404 17.25 7.25848 17.1798 5.1911 17.1136L5.12715 19.1126C7.19769 19.1789 9.90647 19.25 12.0005 19.25L12.0005 17.25ZM2.8622 5.19498C2.90954 3.93324 3.92334 2.92692 5.1911 2.88636L5.12715 0.887381C2.81502 0.961356 0.950565 2.80253 0.863605 5.11999L2.8622 5.19498ZM0.863604 14.88C0.950564 17.1975 2.81502 19.0386 5.12715 19.1126L5.1911 17.1136C3.92334 17.0731 2.90954 16.0668 2.8622 14.805L0.863604 14.88ZM21.1388 14.805C21.0914 16.0668 20.0776 17.0731 18.8099 17.1136L18.8738 19.1126C21.186 19.0386 23.0504 17.1975 23.1374 14.88L21.1388 14.805ZM18.8099 2.88636C20.0776 2.92692 21.0914 3.93324 21.1388 5.19498L23.1374 5.11999C23.0504 2.80253 21.186 0.961356 18.8738 0.887382L18.8099 2.88636Z"
            fill="#8F8F8F"
          ></path>
          <rect
            x="5.5"
            y="5.12207"
            width="5.85"
            height="1.95"
            rx="0.975"
            fill="#8F8F8F"
          ></rect>
          <rect
            x="5.5"
            y="9.02344"
            width="5.85"
            height="1.95"
            rx="0.975"
            fill="#8F8F8F"
          ></rect>
          <rect
            x="5.5"
            y="12.9248"
            width="5.85"
            height="1.95"
            rx="0.975"
            fill="#8F8F8F"
          ></rect>
          <rect
            x="12.6509"
            y="5.12207"
            width="5.85"
            height="1.95"
            rx="0.975"
            fill="#8F8F8F"
          ></rect>
          <rect
            x="12.6509"
            y="9.02344"
            width="5.85"
            height="1.95"
            rx="0.975"
            fill="#8F8F8F"
          ></rect>
          <rect
            x="12.6509"
            y="12.9248"
            width="5.85"
            height="1.95"
            rx="0.975"
            fill="#8F8F8F"
          ></rect>
        </svg>
      ),
      actionIcon: (
        <svg
          width="48"
          height="48"
          viewBox="0 0 48 48"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M16.4517 37C15.0283 36.9773 13.5916 36.9488 12.2178 36.9183C9.16956 36.8506 7.64544 36.8167 6.41564 36.1889C5.31269 35.6258 4.35358 34.6681 3.78881 33.566C3.15908 32.3372 3.12349 30.8373 3.0523 27.8376C3.02068 26.5048 3 25.1888 3 24.0259C3 22.8629 3.02068 21.5469 3.0523 20.2142C3.12349 17.2144 3.15908 15.7145 3.78881 14.4857C4.35358 13.3836 5.31269 12.4259 6.41564 11.8628C7.64544 11.235 9.16959 11.2011 12.2179 11.1334C15.4835 11.0609 19.1045 11 22.0566 11C25.6932 11 30.345 11.0924 34.0971 11.1852C35.0534 11.2088 35.5316 11.2206 35.9699 11.2967C38.4232 11.7225 40.436 13.7225 40.8775 16.173C40.9564 16.6107 40.9709 17.078 41 18.0126V18.0126V18.5916"
            stroke="#999999"
            stroke-width="2.75"
            stroke-linecap="round"
          ></path>
          <path
            d="M10 24.0001H19"
            stroke="#999999"
            stroke-width="2.75"
            stroke-linecap="round"
          ></path>
          <path
            d="M10 30H16"
            stroke="#999999"
            stroke-width="2.75"
            stroke-linecap="round"
          ></path>
          <path
            d="M36.8607 32.1238C36.8607 34.2665 35.1086 36.0036 32.9472 36.0036C30.7859 36.0036 29.0337 34.2665 29.0337 32.1238C29.0337 29.981 30.7859 28.2439 32.9472 28.2439C35.1086 28.2439 36.8607 29.981 36.8607 32.1238Z"
            stroke="#999999"
            stroke-width="2.75"
            stroke-linecap="round"
            stroke-linejoin="round"
          ></path>
          <path
            d="M21.009 33.4638C20.6293 32.6114 20.6293 31.636 21.009 30.7836C23.0327 26.241 27.6168 23.0708 32.9478 23.0708C38.2788 23.0708 42.8629 26.2411 44.8866 30.7837C45.2664 31.6361 45.2664 32.6115 44.8866 33.4639C42.8629 38.0065 38.2788 41.1767 32.9478 41.1767C27.6168 41.1767 23.0327 38.0064 21.009 33.4638Z"
            stroke="#999999"
            stroke-width="2.75"
            stroke-linecap="round"
            stroke-linejoin="round"
          ></path>
        </svg>
      ),
      actionText: "Recovery Phrase",
      actionDisplayText: "Secret Recovery Phrase",
      actionDescription:
        "Your Secret Recovery Phrase is the key used to back up your wallet. Keep it secret at all times.",
      listItems: [
        {
          icon: (
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M3.08654 6.41197C3.13639 5.95634 3.5287 5.62685 3.98583 5.59345C6.79146 5.38847 9.00727 4.30249 11.0607 2.59943C11.4593 2.26879 12.0405 2.26879 12.4391 2.59944C14.4925 4.30249 16.7083 5.38846 19.5139 5.59345C19.9711 5.62685 20.3634 5.95638 20.4132 6.41203C20.4704 6.93468 20.4998 7.46586 20.4998 8.00402C20.4998 13.9495 17.4683 19.0446 12.4223 21.1815C11.9928 21.3634 11.507 21.3634 11.0775 21.1815C6.03148 19.0446 3 13.9495 3 8.00402C3 7.46584 3.02936 6.93464 3.08654 6.41197Z"
                stroke="#A5A5A5"
                strokeWidth="2"
              ></path>
              <path
                d="M8.49097 11.7295L10.8281 14.0359L15.5023 9.4232"
                stroke="#A5A5A5"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></path>
            </svg>
          ),
          listText: "Keep your Secret Phrase safe",
          height: 440,
        },
        {
          icon: (
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M1.75 12L2.75 12L1.75 12ZM12 20.25L12 21.25L12 20.25ZM5.15863 20.1131L5.19061 19.1136L5.15863 20.1131ZM22.1376 7.15748L23.1369 7.11999L22.1376 7.15748ZM2.86171 16.805C2.80378 15.2613 2.75 13.4384 2.75 12L0.75 12C0.75 13.4772 0.804975 15.3306 0.863116 16.88L2.86171 16.805ZM2.75 12C2.75 10.5616 2.80378 8.73874 2.86171 7.19498L0.863117 7.11999C0.804975 8.66944 0.75 10.5228 0.75 12L2.75 12ZM5.19061 4.88636C7.25799 4.82021 9.93991 4.75 12 4.75L12 2.75C9.90598 2.75 7.1972 2.82114 5.12666 2.88738L5.19061 4.88636ZM12 4.75C14.0601 4.75 16.742 4.82021 18.8094 4.88636L18.8733 2.88738C16.8028 2.82114 14.094 2.75 12 2.75L12 4.75ZM21.1383 7.19498C21.1962 8.73874 21.25 10.5616 21.25 12L23.25 12C23.25 10.5228 23.195 8.66944 23.1369 7.11999L21.1383 7.19498ZM21.25 12C21.25 13.4384 21.1962 15.2613 21.1383 16.805L23.1369 16.88C23.195 15.3306 23.25 13.4772 23.25 12L21.25 12ZM18.8094 19.1136C16.742 19.1798 14.0601 19.25 12 19.25L12 21.25C14.094 21.25 16.8028 21.1789 18.8733 21.1126L18.8094 19.1136ZM12 19.25C9.93991 19.25 7.25799 19.1798 5.19061 19.1136L5.12666 21.1126C7.1972 21.1789 9.90598 21.25 12 21.25L12 19.25ZM2.86171 7.19498C2.90906 5.93324 3.92285 4.92692 5.19061 4.88636L5.12666 2.88738C2.81454 2.96136 0.950076 4.80253 0.863117 7.11999L2.86171 7.19498ZM0.863116 16.88C0.950076 19.1975 2.81454 21.0386 5.12666 21.1126L5.19061 19.1136C3.92285 19.0731 2.90906 18.0668 2.86171 16.805L0.863116 16.88ZM21.1383 16.805C21.0909 18.0668 20.0772 19.0731 18.8094 19.1136L18.8733 21.1126C21.1855 21.0386 23.0499 19.1975 23.1369 16.88L21.1383 16.805ZM18.8094 4.88636C20.0772 4.92692 21.0909 5.93324 21.1383 7.19498L23.1369 7.11999C23.0499 4.80253 21.1855 2.96136 18.8733 2.88738L18.8094 4.88636Z"
                fill="#A5A5A5"
              ></path>
              <rect
                x="5.49951"
                y="7.12207"
                width="5.85"
                height="1.95"
                rx="0.975"
                fill="#A5A5A5"
              ></rect>
              <rect
                x="5.49951"
                y="11.0234"
                width="5.85"
                height="1.95"
                rx="0.975"
                fill="#A5A5A5"
              ></rect>
              <rect
                x="5.49951"
                y="14.9248"
                width="5.85"
                height="1.95"
                rx="0.975"
                fill="#A5A5A5"
              ></rect>
              <rect
                x="12.6504"
                y="7.12207"
                width="5.85"
                height="1.95"
                rx="0.975"
                fill="#A5A5A5"
              ></rect>
              <rect
                x="12.6504"
                y="11.0234"
                width="5.85"
                height="1.95"
                rx="0.975"
                fill="#A5A5A5"
              ></rect>
              <rect
                x="12.6504"
                y="14.9248"
                width="5.85"
                height="1.95"
                rx="0.975"
                fill="#A5A5A5"
              ></rect>
            </svg>
          ),
          listText: "Don't share it with anyone else",
        },
        {
          icon: (
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M12 2.99976C7.02944 2.99976 3 7.02919 3 11.9998C3 16.9703 7.02944 20.9998 12 20.9998C16.9706 20.9998 21 16.9703 21 11.9998C21 7.02919 16.9706 2.99976 12 2.99976Z"
                stroke="#A5A5A5"
                strokeWidth="2.2"
              ></path>
              <path
                d="M5.63599 5.63602L18.3639 18.3639"
                stroke="#A5A5A5"
                strokeWidth="2.2"
              ></path>
            </svg>
          ),
          listText: "If you lose it, we can't recover it",
        },
      ],
      type: "netural",
      content: 1,
      height: 440,
    },
    {
      icon: (
        <svg
          width="21"
          height="20"
          viewBox="0 0 21 20"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M11.6324 11.2514L11.8827 7.49726C11.9232 6.88978 11.4414 6.37476 10.8325 6.37476C10.2237 6.37476 9.74185 6.88978 9.78235 7.49726L10.0326 11.2514C10.0607 11.6725 10.4105 11.9998 10.8325 11.9998C11.2546 11.9998 11.6043 11.6725 11.6324 11.2514Z"
            fill="#FF3F3F"
          ></path>
          <circle cx="10.8328" cy="14.0623" r="0.9375" fill="#FF3F3F"></circle>
          <path
            d="M8.71062 3.09582C9.7307 1.5843 11.9348 1.5843 12.9549 3.09582C14.1585 4.87924 15.6235 7.09937 16.6453 8.81189C17.6058 10.4217 18.6773 12.4256 19.5531 14.1178C20.416 15.7849 19.2611 17.7558 17.3855 17.8327C15.3163 17.9175 12.8085 17.9994 10.8328 17.9994C8.85699 17.9994 6.34926 17.9175 4.28004 17.8327C2.40438 17.7558 1.24949 15.7849 2.11241 14.1178C2.98825 12.4256 4.05975 10.4217 5.02026 8.81189C6.04203 7.09937 7.50705 4.87924 8.71062 3.09582Z"
            stroke="#FF3F3F"
            strokeWidth="2"
          ></path>
        </svg>
      ),
      actionIcon: (
        <svg
          width="48"
          height="48"
          viewBox="0 0 48 48"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M22 43C32.4934 43 41 34.4934 41 24C41 13.5066 32.4934 5 22 5C11.5066 5 3 13.5066 3 24C3 34.4934 11.5066 43 22 43Z"
            stroke="#FF3F3F"
            stroke-width="2.75"
            stroke-linecap="round"
            stroke-linejoin="round"
          ></path>
          <path
            d="M23.55 26.5008L23.9334 14.9989C23.9698 13.9059 23.0936 13 22 13C20.9064 13 20.0302 13.9059 20.0666 14.9989L20.45 26.5008C20.4779 27.3367 21.1636 28 22 28C22.8364 28 23.5221 27.3367 23.55 26.5008Z"
            fill="#FF3F3F"
          ></path>
          <circle cx="21.9866" cy="33.2991" r="1.9866" fill="#FF3F3F"></circle>
        </svg>
      ),
      actionText: "Remove Wallet",
      actionDisplayText: "Are you sure",
      actionDescription:
        "You haven’t backed up your wallet yet. If you remove it, you could lose access forever. We suggest tapping and backing up your wallet first with a valid recovery method.",
      type: "danger",
      content: 2,
      height: 380,
    },
  ]);

  React.useEffect(() => {
    if (content > -1) {
      setTrayHeight(actions[content].height ? actions[content].height : 0);
    }
  }, [content]);

  return (
    <StyledTrayContainer isTrayOpen={props.isTrayOpen}>
      <StyledTrayOverlay />

      {props.isTrayOpen == true ? (
        <StyledTray
          style={{ overflow: "hidden" }}
          initial={{ height: 0, bottom: -100 }}
          animate={{ height: content == -1 ? 295 : trayHeight, bottom: 20 }}
          transition={{
            duration: 0.3,
            delay: 0.2,
            ease: [0.16, 1, 0.3, 1],
          }}
        >
          <AnimatePresence>
            {content === -1 && (
              <motion.div
                key="options"
                ref={contentRef}
                variants={trayVariants}
                initial="initial"
                animate="animate"
                exit="exit"
              >
                <StyledTrayContent>
                  <TrayHead
                    heading="Options"
                    setIsTrayOpen={props.setIsTrayOpen}
                  />
                  <TraySpacedContainer>
                    {actions.map((y) => (
                      <TrayItemAction
                        height={y.height}
                        setTrayHeight={setTrayHeight}
                        key={y.actionText}
                        icon={y.icon}
                        actionText={"View " + y.actionText}
                        type={y.type}
                        content={y.content}
                        setContent={setContent}
                      />
                    ))}
                  </TraySpacedContainer>
                </StyledTrayContent>
              </motion.div>
            )}

            {content > -1 && (
              <motion.div
                key="detail"
                ref={contentRef}
                variants={contentVariants}
                initial="initial"
                animate="animate"
                exit="exit"
              >
                <TrayOptionDetailContent
                  icon={actions[content].actionIcon}
                  heading={actions[content].actionDisplayText}
                  description={actions[content].actionDescription}
                  listItems={
                    actions[content].listItems ? actions[content].listItems : []
                  }
                  setContent={setContent}
                  setIsTrayOpen={props.setIsTrayOpen}
                  type={actions[content].type}
                />
              </motion.div>
            )}
          </AnimatePresence>
        </StyledTray>
      ) : (
        <></>
      )}
    </StyledTrayContainer>
  );
};

type Props = {
  heading: string;
  description?: string;
  active: boolean;
  icon?: React.ReactNode;
  setIsTrayOpen: () => void;
};

const TrayHead: React.FC<Props> = (props: Props) => {
  return (
    <StyledTrayHead active={props.active}>
      <StyledTrayHeadContent>
        <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          <div>{props.icon}</div>
          <div>{props.heading}</div>
        </div>
        <StyledClose onClick={() => props.setIsTrayOpen(false)}>
          <svg
            width="12"
            height="12"
            viewBox="0 0 12 12"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M10.4854 1.99998L2.00007 10.4853"
              stroke="#999999"
              strokeWidth="3"
              strokeLinecap="round"
              strokeLinejoin="round"
            ></path>
            <path
              d="M10.4854 10.4844L2.00007 1.99908"
              stroke="#999999"
              strokeWidth="3"
              strokeLinecap="round"
              strokeLinejoin="round"
            ></path>
          </svg>
        </StyledClose>
      </StyledTrayHeadContent>
      {props.description ? (
        <StyledTrayOptionDescription>
          {props.description}
        </StyledTrayOptionDescription>
      ) : (
        <></>
      )}
    </StyledTrayHead>
  );
};

type ActionProps = {
  icon: any;
  actionText: string;
  type: string;
  content: number;
  setContent: any;
  height: number;
  setTrayHeight: any;
};

const TrayItemAction: React.FC<ActionProps> = (props: ActionProps) => {
  const handleTrayItemAction = () => {
    props.setContent(props.content);
  };
  return (
    <StyledTrayItemAction
      type={props.type}
      onClick={() => handleTrayItemAction()}
    >
      {props.icon} <div>{props.actionText}</div>
    </StyledTrayItemAction>
  );
};

type TrayOptionProps = {
  heading: string;
  description: string;
  listItems?: any[];
  setContent: () => void;
  type: string;
  icon: React.ReactNode;
  setIsTrayOpen: () => void;
};

const TrayOptionDetailContent: React.FC<TrayOptionProps> = (
  props: TrayOptionProps,
) => {
  const handleClose = () => {
    props.setContent(-1);
  };
  return (
    <StyledTrayContent>
      <TrayHead
        icon={props.icon}
        active={true}
        heading={props.heading}
        description={props.description}
        setIsTrayOpen={props.setIsTrayOpen}
      />
      <StyledTrayListItems>
        {props.listItems &&
          props.listItems.map((item, index) => (
            <StyledTrayListItem key={index}>
              {item.icon}
              <span>{item.listText}</span>
            </StyledTrayListItem>
          ))}
      </StyledTrayListItems>
      <TrayFooterContainer>
        <AnimatedButton type="netural" onClick={() => handleClose()}>
          Cancel
        </AnimatedButton>
        {props.type == "danger" ? (
          <AnimatedButton type="danger" onClick={() => handleClose()}>
            Continue
          </AnimatedButton>
        ) : (
          <AnimatedButton type="primary" onClick={() => handleClose()}>
            Reveal
          </AnimatedButton>
        )}
      </TrayFooterContainer>
    </StyledTrayContent>
  );
};

const Container = styled.div`
  width: 100%;
  height: 100%;
  display: flex;
  align-items: center;
  justify-content: center;
  --color: #fff;
  --background: #0093ff;
  --background-overlay: rgba(0, 0, 0, 0.1);
  --shadow-color: #fff;
  --border-color: #0093ff;
  --netural: #f7f8f9;
  --dangerText: #ff3f40;
  --danger: #fff0f0;
  --text: #000;
`;

const AnimatedButton = styled.button<{ type?: string }>`
  cursor: pointer;
  transition: 0.18s ease;
  transition-property: box-shadow, background-color, transform;
  will-change: box-shadow, background-color, transform;
  position: relative;
  display: inline-block;
  padding: 0.891rem 1.25rem;
  font-size: 19px;
  font-weight: 550;
  letter-spacing: -0.00813rem;
  white-space: nowrap;
  user-select: none;
  border-radius: 9999px;
  width: 100%;
  color: ${(props) =>
    props.type === "netural"
      ? "var(--text)"
      : props.type === "danger"
        ? "var(--dangerText)"
        : "#fff"};
  background: ${(props) =>
    props.type === "netural"
      ? "var(--netural)"
      : props.type === "danger"
        ? "var(--danger)"
        : "var(--background)"};
  box-shadow:
    0 0 0 0 var(--border-color),
    inset 0 0 0 0 var(--shadow-color);
  outline: 0;
  transition: all 0.2s ease;
  border: 0px;

  &:after {
    z-index: 1;
    content: "";
    position: absolute;
    inset: 0;
    background-color: var(--background-overlay);
    border-radius: inherit;
    opacity: 0;
    transition: inherit;
    transition-property: opacity, background-color;
  }

  &:active {
    transform: scale(0.95);
  }
`;

const StyledTrayContainer = styled(motion.div)<{ isTrayOpen: boolean }>`
  position: fixed;
  width: 100%;
  height: 100%;
  display: ${(props) => (props.isTrayOpen == true ? "block" : "none")};
`;

const StyledTrayOverlay = styled.div`
  position: fixed;
  width: 100%;
  height: 100%;
  background-color: #000;
  opacity: 0.5;
  z-index: 1;
`;

const StyledTray = styled(motion.div)`
  position: fixed;
  bottom: 20px;
  width: 360px;
  background-color: #fff;
  z-index: 100000;
  left: 50%;
  transform: translateX(-50%);
  border-radius: 35px;
`;

const StyledTrayContent = styled.div`
  padding-top: 0.625rem;
  padding-bottom: 1.5rem;
  padding-left: 1.5rem;
  padding-right: 1.5rem;
`;

const StyledTrayHead = styled.div<{ active: boolean }>`
  border-bottom: 1px solid #f7f7f7;
  padding-left: -0.5rem;
  -webkit-tap-highlight-color: transparent;
  display: flex;
  margin-bottom: 1rem;
  font-weight: 600;
  font-size: ${(props) => (props.active ? "22px" : "17px")};
  flex-direction: column;
  margin-top: 21px;
  padding-bottom: 1.5rem;
`;

const StyledTrayHeadContent = styled.div`
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: space-between;
`;

const StyledClose = styled.div`
  transition-property: transform;
  transition-timing-function: cubic-bezier(0.4, 0, 0.2, 1);
  transition-duration: 0.15s;
  color: #949595;
  background-color: var(--netural);
  border-radius: 9999px;
  justify-content: center;
  align-items: center;
  display: flex;
  z-index: 10;
  position: absolute;
  width: 2rem;
  height: 2rem;
  right: 20px;
  cursor: pointer;
`;

const StyledTrayItemAction = styled(motion.div)<{ type: string }>`
  font-weight: 600;
  -webkit-tap-highlight-color: transparent;
  transition-property: transform;
  transition-timing-property: cubic-bezier(0.4, 0, 0.2, 1);
  transition-duration: 0.15s;
  color: ${(props) =>
    props.type == "danger" ? "var(--dangerText)" : "var(--text)"};
  font-size: 17px;
  padding-left: 1rem;
  padding-right: 1rem;
  background-color: ${(props) =>
    props.type == "danger" ? "var(--danger)" : "var(--netural)"};
  border-radius: 15px;
  gap: 15px;
  display: flex;
  align-items: center;
  width: 100%;
  height: 3rem;
  cursor: pointer;
  &:active {
    transform: scale(0.97);
  }
`;

const TraySpacedContainer = styled.div`
  display: flex;
  flex-direction: column;
  gap: 15px;
`;

const StyledTrayOptionDescription = styled.p`
  line-height: 24px;
  color: #999;
  -webkit-tap-highlight-color: transparent;
  font-size: 17px;
  font-weight: 600;
  margin-top: 0.75rem;
`;

const StyledTrayListItems = styled.div`
  display: flex;
  align-items: flex-start;
  flex-direction: column;
  gap: 15px;
`;
const StyledTrayListItem = styled.div`
  color: #999;
  font-weight: 600;
  font-size: 15px;
  gap: 0.75rem;
  align-items: center;
  display: flex;
  font-variant-ligatures: none;
`;

const TrayFooterContainer = styled.div`
  margin-top: 1.75rem;
  display: flex;
  flex-direction: row;
  gap: 15px;
  align-items: center;
`;
